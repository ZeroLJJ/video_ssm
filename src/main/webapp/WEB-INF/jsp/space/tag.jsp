...
<div id="space-body" style="width: 86%;left: 7%;position: relative">
    ...
    <div class="s-space">
        <div class="wrapper clearfix" id="page-subs">
            <div class="col-full">
                <div class="tabs">
                    <a href="<%=path%>/space/${spaceUser.uid}/follow">关注 </a>
                    <a href="<%=path%>/space/${spaceUser.uid}/fan">粉丝 </a>
                    <a href="<%=path%>/space/${spaceUser.uid}/tag" class="active">标签 </a>
                </div>
                <div class="section subs">
                    <div class="content clearfix">
                        <c:forEach var="item" items="${tagList}">
                        <div class="mini-item">
                            <a target="_blank" class="cover" href="<%=path%>/video/all?tag=${item.tagBO.tid}" title="${item.tagBO.tname}">
                                <img src="<%=path%>/templete/bilibili/images/transparent.gif" alt="${item.tagBO.tname}">
                            </a>
                            <div class="detail" title="${item.tagBO.tname}">
                                <a target="_blank" href="<%=path%>/video/all?tag=${item.tagBO.tid}" title="${item.tagBO.tname}">${item.tagBO.tname}</a>
                            </div>
                            <c:if test="${isOwner == true}">
                            <div class="action">
                                <span class="sub-action unsub" data-tag="${item.tagBO.tid}" data-user="${spaceUser.uid}">取消订阅</span>
                            </div>
                            </c:if>
                        </div>
                        </c:forEach>
