...
<nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> 全员列表
...
<div class="page-container">
    <div class="text-c">
        <form action="<%=path%>/manage/admin-list" method="post">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <span class="r">共有数据：<strong>${fn:length(adminList)}</strong> 条</span>
    </div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <c:forEach var="item" items="${adminList}">
                <tr class="text-c">
                    ...
                    <td>${item.uid}</td>
                    <td><img src="<%=path%>/images/${item.uimg}" style="border-radius: 32px;height: 64px;width: 64px;" alt="暂无头像"></td>
                    ...
                    <td class="td-level">
                        <c:if test="${item.ulevel=='0'}"><span class="label label-primary radius">超级管理员</span></c:if>
                        <c:if test="${item.ulevel=='1'}"><span class="label label-secondary radius">管理员</span></c:if>
                        <c:if test="${item.ulevel=='2'}"><span class="label label-success radius">普通用户</span></c:if>
                    </td>
                    <td class="td-manage">
                        <c:if test="${item.ulevel!='0'}">
                        ...
                        <a class="down" <c:if test="${item.ulevel=='1'}">style="display: inline-block"</c:if>
                           data-id="${item.uid}" href="javascript:;" title="撤销">
                            <i class="Hui-iconfont">&#xe674;</i>
                        </a>
                        <a class="up" <c:if test="${item.ulevel=='2'}">style="display: inline-block"</c:if>
                           data-id="${item.uid}" href="javascript:;" title="任命">
                            <i class="Hui-iconfont">&#xe679;</i>
                        </a>
